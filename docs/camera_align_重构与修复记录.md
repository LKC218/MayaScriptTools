...
> 当前最终脚本：`脚本/相机工具/camera_align_one_click_standalone_final.py`
...
### 问题 11：旋转步长设置无法生效（v7 修复）

表现：在 UI 的"旋转设置"中修改旋转步长（如改为 `45°`），点击顺/逆时针旋转按钮时生效，但通过快捷键 `Alt+W` / `Alt+E` 触发时始终使用默认 `22.5°`。

原因：快捷键调用链路不经过 UI 步长读取。

UI 按钮调用路径（正确）：

```python
# UI 按钮的 lambda 中显式传入了 get_rotate_step_from_ui() 的返回值
lambda *a: _safe_call(
    rotate_aligned_camera_clockwise,
    ...,
    get_rotate_step_from_ui(),  # ← 读取了 UI 步长
)
```
...
`rotate_aligned_camera_clockwise()` 不传参数，函数签名为 `step=22.5`，硬编码默认值，永远不读取 UI。

修复：将三个旋转便捷函数的默认参数从 `22.5` 改为 `None`，当参数为 `None` 时自动调用 `get_rotate_step_from_ui()` 读取 UI 中的步长值：
...
这样无论从 UI 按钮、快捷键还是其他入口调用，都能正确读取用户设置的旋转步长。

---

### 问题 12：旋转操作无动画过渡（v7 修复）

表现：对齐到面时有平滑飞行动画（100 步四元数插值），但顺/逆时针旋转是瞬间跳变，视觉体验割裂。

原因：`rotate_cam()` 方法计算目标 up 方向后直接调用 `camera.set()` 赋值，没有插值过渡。

对比：

| 对比项 | 对齐动画 `transform_cam` | 旋转操作 `rotate_cam`（修复前） |
|---|---|---|
| 过渡方式 | 100 步四元数球面插值（slerp），5ms/步 | 无动画，直接 `camera.set()` 赋值 |
| 视觉效果 | 平滑飞行到目标面 | 瞬间跳变 |
| 定时器 | `MTimerMessage` 回调 | 无 |

修复：复用对齐动画的 `MTimerMessage` + 四元数 slerp 模式，为旋转添加平滑过渡。

1. `_AlignState` 新增旋转动画状态字段：

```python
self.rotate_up_start = None      # 旋转动画起始 up 方向
self.rotate_up_target = None     # 旋转动画目标 up 方向
self.rotate_step_index = 0       # 旋转动画当前步数
self.is_rotating = False         # 旋转动画进行中标记
```

2. `Camera_Align` 新增常量：

```python
ROTATE_INTERP_STEPS = 40  # 旋转插值步数（约 0.2 秒，比对齐的 100 步更干脆）
```

3. 新增 `_rotate_cam_step` 定时回调方法：
...
4. 重构 `rotate_cam` 方法：计算目标 up 方向后启动定时动画，而非直接赋值。

5. 并发防护：
   - 旋转动画进行中再次触发旋转 → 先完成当前动画（跳到目标 up），再启动新动画。
   - 恢复透视时 → `_clear_timer()` + 重置 `is_rotating = False`。

---

## 5. 当前最终脚本结构说明

当前最终脚本（v7 Standalone）采用自包含设计，不再依赖预装的 `camera_align.py`。脚本内部嵌入了完整的 `camera_align.py` 源码（`CAMERA_ALIGN_SOURCE` 字符串），安装时自动写入用户脚本目录。

核心流程：

```python
install()
  ├── _write_camera_align()       → 将内嵌源码写入 ~/maya/scripts/camera_align.py
  │     └── _backup_existing_file() → 如有旧文件，备份为 camera_align_backup_时间戳.py
  ├── _import_camera_align()      → 刷新缓存并 import/reload 模块
  └── module.show_camera_align_ui() → 打开 UI 窗口
```

支持两种启动方式：

1. **拖放**：通过 `onMayaDroppedPythonFile` 钩子自动触发安装。
2. **脚本编辑器执行**：通过 `if __name__ == "__main__"` 入口。

旧版 v6 补丁块追加方式已废弃，v7 直接覆盖写入完整文件。
...
```text
workspaceControl（可停靠面板，回退到普通 window）
└── scrollLayout
    └── columnLayout
        ├── 标题区
        ├── 状态条
...
```text
脚本/相机工具/camera_align_one_click_standalone_final.py
...
```text
脚本/相机工具/uninstall_camera_align.py
...
| 快捷键旋转步长不生效 | 便捷函数默认参数硬编码 `22.5`，快捷键无参调用不读取 UI | 默认参数改为 `None`，`None` 时调用 `get_rotate_step_from_ui()` |
| 旋转瞬间跳变无动画 | `rotate_cam()` 直接 `camera.set()` 赋值，无插值过渡 | 新增 `_rotate_cam_step` 定时回调，40 步四元数 slerp 插值 |
...
- Maya 2022+ 源码化。
- 现代化 UI（workspaceControl 可停靠面板 + 回退普通窗口）。
- Shelf 按钮入口。
- 快捷键入口（Alt+Q/W/E/R + Ctrl+Alt 备用）。
- 备用快捷键入口。
- 安装成功提示简化。
- UI 裁剪修复。
- MEL / Python 热键解析问题修复。
- Hotkey Set 锁定问题规避。
- nameCommand 旧命令污染规避。
- 自包含安装（v7 Standalone：内嵌完整 `camera_align.py`，无需预装）。
- 旋转步长设置全局生效（v7 修复：快捷键也读取 UI 步长）。
- 旋转平滑动画过渡（v7 新增：40 步四元数 slerp 插值，与对齐动画风格一致）。
- 视窗 HUD 快捷键提示。
- 快捷键自定义 UI（按键 + 修饰键 + 冲突检测 + 配置持久化）。

当前仍需注意：

- 纯 `Alt` 快捷键在部分系统中可能被截获，因此保留 `Ctrl+Alt` 备用。
- 原始 `align_obj_to_cam_plane()` 仍未实现，仅保留占位。
- 旋转动画进行中再次触发旋转会先跳到当前目标再启动新动画，这是有意设计以避免状态混乱。
